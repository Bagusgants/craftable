...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('update_at'), 'has-success': fields.update_at && fields.update_at.valid }">
    <label for="update_at" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.mahasiswa.columns.update_at') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.update_at" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('update_at'), 'form-control-success': fields.update_at && fields.update_at.valid}" id="update_at" name="update_at" placeholder="{{ trans('admin.mahasiswa.columns.update_at') }}">
        <div v-if="errors.has('update_at')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('update_at') }}</div>
    </div>
</div>
